...
class ChameleonSystemSecurityExtension extends ConfigurableExtension
{
    ...
    public function loadInternal(array $mergedConfig, ContainerBuilder $container)
    {
        ...
        $newUserConfig->setArgument('$domainToBaseUserMapping', $mergedConfig['google_login']['domain_to_base_user_mapping']);

        $authenticator = $container->getDefinition(GoogleAuthenticator::class);
        $authenticator->setArgument('$allowedDomains', array_keys($mergedConfig['google_login']['domain_to_base_user_mapping']));
    }
}
