...
if (is_array($aValidationErrors) && count($aValidationErrors) > 0) {
    ?>
        <div style="margin:20px 0 10px 0px;font-weight: bold;"><?php echo TGlobal::OutHTML(ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_csv2sql.error.validation_errors')); ?>:</div>
...
if (is_array($aImportErrors) && count($aImportErrors) > 0) {
    ?>
        <div style="margin:20px 0 10px 0px;font-weight: bold;"><?php echo TGlobal::OutHTML(ChameleonSystem\CoreBundle\ServiceLocator::get('translator')->trans('chameleon_system_csv2sql.error.import_errors')); ?>:</div>
