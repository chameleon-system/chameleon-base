...
class NavigationTree extends \MTPkgViewRendererAbstractModuleMapper
{
    ...
    private string $treeTableSortField = 'entry_sort';
    ...
    private BackendTreeNodeFactory $backendTreeNodeFactory;
    ...
    private NestedSetHelperFactoryInterface $nestedSetHelperFactory;

    private \TTools $tools;

    private \TGlobal $global;

    private FieldTranslationUtil $fieldTranslationUtil;
    ...
    private function createRootTree(): array
    {
        ...
        if ('' === $this->currentPageId && \TCMSTreeNode::TREE_ROOT_ID === $this->rootNodeId) {
    ...
    public function GetHtmlHeadIncludes()
    {
        ...
        $includes[] = sprintf('<script src="%s" type="text/javascript"></script>', $this->global->GetStaticURLToWebLib('/javascript/jsTree/3.3.17/jstree.min.js'));
        $includes[] = sprintf('<script src="%s" type="text/javascript"></script>', $this->global->GetStaticURLToWebLib('/javascript/navigationTree.js?v1'));
        ...
        $includes[] = sprintf('<link rel="stylesheet" href="%s">', $this->global->GetStaticURLToWebLib('/javascript/jsTree/3.3.17/themes/default/style.min.css'));
