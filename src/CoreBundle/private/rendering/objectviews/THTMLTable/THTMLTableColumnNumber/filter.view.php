...
<div class="filterbox">
    <div style="display:none;" class="filterPopUp" id="<?=$sFilterPopupId; ?>">
        <form name="" accept-charset="utf-8" method="post" action="<?=$oOwningTable->GetFilterURL($oColumn); ?>">
            <input type="text"
                   name="<?=TGlobal::OutHTML($oOwningTable->sListIdentKey); ?>[<?=THTMLTable::URL_PARAM_SEARCH; ?>][<?=TGlobal::OutHTML($oColumn->sColumnAlias); ?>][<?=THTMLTableColumnNumber::FILTER_FROM; ?>]"
                   value="<?=TGlobal::OutHTML($sStartVal); ?>"/>
            <input type="text"
                   name="<?=TGlobal::OutHTML($oOwningTable->sListIdentKey); ?>[<?=THTMLTable::URL_PARAM_SEARCH; ?>][<?=TGlobal::OutHTML($oColumn->sColumnAlias); ?>][<?=THTMLTableColumnNumber::FILTER_TO; ?>]"
