...
if (is_array($searchFilter)) {
    if (array_key_exists(THTMLTableColumnNumber::FILTER_FROM, $searchFilter)) {
        $sStartVal = $searchFilter[THTMLTableColumnNumber::FILTER_FROM];
    }
    if (array_key_exists(THTMLTableColumnNumber::FILTER_TO, $searchFilter)) {
        $sEndVal = $searchFilter[THTMLTableColumnNumber::FILTER_TO];
    }
}
...
<div class="filterbox">
    <div style="display:none;" class="filterPopUp" id="<?php echo $sFilterPopupId; ?>">
        <form name="" accept-charset="utf-8" method="post" action="<?php echo $oOwningTable->GetFilterURL($oColumn); ?>">
            <input type="text"
                   name="<?php echo TGlobal::OutHTML($oOwningTable->sListIdentKey); ?>[<?php echo THTMLTable::URL_PARAM_SEARCH; ?>][<?php echo TGlobal::OutHTML($oColumn->sColumnAlias); ?>][<?php echo THTMLTableColumnNumber::FILTER_FROM; ?>]"
                   value="<?php echo TGlobal::OutHTML($sStartVal); ?>"/>
            <input type="text"
                   name="<?php echo TGlobal::OutHTML($oOwningTable->sListIdentKey); ?>[<?php echo THTMLTable::URL_PARAM_SEARCH; ?>][<?php echo TGlobal::OutHTML($oColumn->sColumnAlias); ?>][<?php echo THTMLTableColumnNumber::FILTER_TO; ?>]"
