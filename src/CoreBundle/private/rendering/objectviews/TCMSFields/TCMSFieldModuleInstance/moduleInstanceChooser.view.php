...
<script type="text/javascript">
    ...
    function Rename(fieldName, instanceName) {
        var sNewName = window.prompt('<?=TGlobal::OutJS($translator->trans('chameleon_system_core.template_engine.prompt_instance_name')); ?>:', instanceName);
    ...
    function CreateModuleInstance(fieldName, moduleID, sView) {
        ...
        var sNewName = window.prompt('<?=TGlobal::OutJS($translator->trans('chameleon_system_core.template_engine.prompt_instance_name')); ?>:', '<?=$sRecordName; ?>');
    ...
    function DeleteModuleInstance(fieldName, moduleInstanceId) {
        if (confirm('<?=TGlobal::OutJS($translator->trans('chameleon_system_core.action.confirm_delete')); ?>')) {
            document.getElementById('cmsModuleMenu').style.display = 'none';
    ...
    function DeleteModuleInstanceFinal(data) {
        ...
        if (data && typeof data === 'object') {
            var message = '<h1><?=TGlobal::OutJS($translator->trans('chameleon_system_core.field_module_instance.error')); ?></h1><?=TGlobal::OutJS($translator->trans('chameleon_system_core.template_engine.error_delete_still_used')); ?><br /><br />';
            data.forEach(item => {
                message += '<div style="padding-bottom: 10px;">';
                message += item.tree;
                message += '<a href="<?=$sPageEditURL; ?>&id=' + item.id + '" target="_parent" class="btn btn-danger"><?=TGlobal::OutJS($translator->trans('chameleon_system_core.cms_module_page_tree.action_edit_page')); ?></a><hr width="95%" />';
                message += '</div>';
            });
        ...
        if (!document.getElementById("cmsModuleMenu")) {
            ...
            <div class="moduleMenuHeader">
                <a href="#" onclick="document.getElementById('cmsModuleMenu').style.display='none'; return false;">
                    <?=TGlobal::OutJS($translator->trans('chameleon_system_core.action.close')); ?>
                </a>
            </div>
...
<div id="<?=$menuPrefix; ?>MenuTree" style="display: none;">
    <ul>
        ...
        <li><a href="#"
                ...
                } ?>><i class="fas fa-edit"></i> <?=$translator->trans('chameleon_system_core.link.edit'); ?></a>
            <ul>
        ...
        ?>
        <li><a href="#;" class="hasChildren" onclick="openMenuLevel(this);return false;"><i class="fas fa-desktop"></i> <?=$translator->trans('chameleon_system_core.template_engine.action_change_template'); ?></a>
            <?php
            echo "<ul>\n";
            $viewCount = 0;
        ...
        <li><a href="#"
               onclick="Rename('<?=TGlobal::OutJS($oField->name); ?>','<?=TGlobal::OutJS($oField->oModuleInstance->sqlData['name']); ?>');return false;"><i class="fab fa-fonticons"></i> <?=$translator->trans('chameleon_system_core.template_engine.slot_rename'); ?></a>
        </li>
        <li><a href="#"
               onclick="DeleteModuleInstance('<?=TGlobal::OutJS($oField->name); ?>','<?=TGlobal::OutJS($oField->oModuleInstance->id); ?>');return false;"><i class="fas fa-file-excel"></i> <?=$translator->trans('chameleon_system_core.template_engine.action_delete_instance_content'); ?></a>
        </li>
        <?php
        }
        ...
        if ($securityHelper->isGranted('CMS_RIGHT_CMS_TEMPLATE_MODULE_EDIT')) {
        ?>
        <li><a href="#" class="hasChildren" onclick="openMenuLevel(this);return false;"><i class="fas fa-plus"></i> <?=$translator->trans('chameleon_system_core.template_engine.action_create_module_instance'); ?></a>
            <ul>
                <?php
