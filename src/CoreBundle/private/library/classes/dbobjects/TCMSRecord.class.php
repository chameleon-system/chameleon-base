...
class TCMSRecord implements IPkgCmsSessionPostWakeupListener
{
    ...
    public function Load($id)
    {
        ...
        if (!empty($id)) {
            if ($this->GetEnableObjectCaching()) {
                ...
                $query .= ' LIMIT 1';
                $this->sqlData = $this->ExecuteSQLQueries($query, [$id])->fetch(PDO::FETCH_ASSOC);
    ...
    {
        $aCacheRelatedTables = [['table' => $this->table, 'id' => $id]];
    ...
    {
        $aCacheKeys = ['sTableName' => $this->table, 'sFieldName' => $sFieldName, 'sFieldValue' => $sFieldValue, 'iLanguageId' => $this->iLanguageId, 'sObjectName' => get_class($this)];
