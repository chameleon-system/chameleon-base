...
if (!empty($sForeignTableName) && TTools::FieldExists($sForeignTableName, $oField->sTableName.'_id')) {
          ...
          ';
                if (is_array($aAdditionalFields) && count($aAdditionalFields) > 0) {
                    foreach ($aAdditionalFields as $sFieldName) {
                        ...
                        if ($oFieldTargetTable->LoadFromFields(array('name' => $sFieldName, 'cms_tbl_conf_id' => TTools::GetCMSTableId($sForeignTableName)))) {
                            $sValue = '';
    ...
    $sHTML .= '
          sAppendHTML += "<div class=\"remove '.TGlobal::OutJS($oField->name).'remove\"><span class=\"removebutton\" onclick=\"$(this).parent().parent().remove();return false;\">'.TGlobal::OutHTML(TGlobal::Translate('chameleon_system_core.field_property_media.remove')).'<\/span><\/div>";
      ...
      <div id="'.TGlobal::OutHTML($oField->name).'">
        ...
        <div class="add '.TGlobal::OutHTML($oField->name).'add"><span class="addbutton" onclick="AddNew'.TGlobal::OutHTML($sForeignTableName).'();return false;">'.TGlobal::OutHTML(TGlobal::Translate('chameleon_system_core.field_property_media.add_new_media')).'</span></div>
        '.$sConnectedRecordsHTML.'
      </div>
    ';
}
