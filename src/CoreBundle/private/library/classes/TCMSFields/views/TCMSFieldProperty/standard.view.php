...
if (!empty($sForeignTableName) && TTools::FieldExists($sForeignTableName, $oField->sTableName.'_id')) {
    ...
    if (count($aRecordsConnected) > 0) {
        foreach ($aRecordsConnected as $aRow) {
            ...
            $sConnectedRecordsHTML .= '<div class="remove '.TGlobal::OutHTML($oField->name).'remove"><span class="removebutton" onclick="$(this).parent().parent().remove();return false;">'.TGlobal::OutHTML(TGlobal::Translate('chameleon_system_core.field_property.remove')).'</span></div></div>';
    ...
    $sHTML .= '
            sAppendHTML += "<div class=\"remove '.TGlobal::OutJS($oField->name).'remove\"><span class=\"removebutton\" onclick=\"$(this).parent().parent().remove();return false;\">'.TGlobal::OutHTML(TGlobal::Translate('chameleon_system_core.field_property.remove')).'<\/span><\/div>";
        ...
        <div id="'.TGlobal::OutHTML($oField->name).'">
          ...
          <div class="add '.TGlobal::OutHTML($oField->name).'add"><span class="addbutton" onclick="AddNew'.TGlobal::OutHTML($sForeignTableName).'();return false;">'.TGlobal::OutHTML(TGlobal::Translate('chameleon_system_core.field_property.add_new')).'</span></div>
          '.$sConnectedRecordsHTML.'
        </div>
      ';
}
