...
class TCMSLogChange
{
    ...
    /**
     * @deprecated in 6.3.9 - use TCMSLogChange::addMessage()
     *
     * inserts a portal based frontend message (german only) if it doesn't exist already or updates an existing autogenerated message.
     *
    ...
    public static function AddFrontEndMessage($sIdentifierName, $sMessage, $sMessageTypeId = '4', $sDescription = '', $sPortalID = '', $sMessageLocationType = 'Core', $sMessageView = 'standard', $language = null)
    {
        ...
        $sCheckQuery = 'SELECT * FROM `cms_message_manager_message` WHERE `name` = :name';
        $result = $connection->fetchAll($sCheckQuery, array(
            'name' => $sIdentifierName,
        ));
        ...
        if (count($result) > 0) {
            $row = $result[0];
            if (false === static::isMessageEmpyOrInsertedAutomatically($row, $languageCode)) {
                return;
            }
    ...
    public static function addMessage(
        ...
        // TODO how to differentiate from backend case (method name)?

        // TODO use language iso directly?

        $languageIso = 'en';
        if (null === $languageId) {
            $language = self::getLanguageService()->getCmsBaseLanguage();
            $languageIso = $language->fieldIso6391;
        } else {
            $language = TdbCmsLanguage::GetNewInstance();
            if (true === $language->Load($languageId)) {
                $languageIso = $language->fieldIso6391;
            } // TODO else logging?
        }
        $portalIds = [];
        ...
        if (null === $portalId) {
            $portalIds = self::getAllPortalIds();
        } else {
            $portalIds[] = $portalId;
        }
        ...
        // There is the "insert TODO case": \TCMSMessageManagerMessage::Render() but only in development mode

        foreach ($portalIds as $onePortalId) {
            $data = self::createMigrationQueryData('cms_message_manager_message', $languageIso)
                ->setFields(
                    [
                        'name' => $identifier,
                        'message' => $message,
                        ...
                        'message_location_type' => 'Core',
                        // TODO there is one with a different type (Customer); and the only one with a different view (layover)
                        'message_view' => 'standard',
                        'cms_portal_id' => $onePortalId,
    ...
    private static function getAllPortalIds(): array
    {
        $connection = self::getDatabaseConnection();

        $rows = $connection->fetchAll('SELECT `id` FROM `cms_portal`');
     ...
     * @return bool
     */
    private static function isMessageEmpyOrInsertedAutomatically(array $row, $languageCode)
    {
