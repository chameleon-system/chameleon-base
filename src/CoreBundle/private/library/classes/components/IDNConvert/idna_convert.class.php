    ...
    class idna_convert
    {
        ...
        public function decode($input, $one_time_encoding = false)
        {
            ...
            if (strpos($input, '@')) { // Maybe it is an email address
                ...
                if (isset($parsed['host'])) {
                    ...
                    $return = (empty($parsed['scheme']) ? '' : $parsed['scheme'].('mailto' == strtolower($parsed['scheme']) ? ':' : '://')).(empty($parsed['user']) ? '' : $parsed['user'].(empty($parsed['pass']) ? '' : ':'.$parsed['pass']).'@').$parsed['host'].(empty($parsed['port']) ? '' : ':'.$parsed['port']).(empty($parsed['path']) ? '' : $parsed['path']).(empty($parsed['query']) ? '' : '?'.$parsed['query']).(empty($parsed['fragment']) ? '' : '#'.$parsed['fragment']);
        ...
        public function _hangul_compose($input)
        {
            ...
            for ($i = 1; $i < $inp_len; ++$i) {
                ...
                if (0 <= $sindex && $sindex < $this->_scount && (0 == $sindex % $this->_tcount) && 0 <= $tindex && $tindex <= $this->_tcount) {
 ...
 * @author  Matthias Sommerfeld <mso@phlylabs.de>
 */
if (!class_exists('Net_IDNA_php4', false)) {
