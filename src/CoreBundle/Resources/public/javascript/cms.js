...
CHAMELEON.CORE.handleTabChanges = function($tabsWrapper) {
    ...
    if ($tabHeaders.length > 0 && $tabContents.length > 0) {
        $(window).on("hashchange", function(event) {
            ...
            if (document.location.hash.length > 0) {
                $currentHeader = $tabHeaders.filter("[href='"+document.location.hash+"']");
                $currentContent = $(document.location.hash);
            } else {
                $currentHeader = $tabHeaders.first();
                $currentContent = $tabContents.first();
            }

            if ($currentHeader.length > 0) {
                $tabHeaders.removeClass("active").removeClass("show");
                $currentHeader.addClass("active").addClass("show");
            }

            if ($currentContent.length > 0) {
                $tabContents.removeClass("active").removeClass("show");
                $currentContent.addClass("active").addClass("show");
            }
        });
    }
};
...
$(function() {
    ...
    if ($tabsWrapper.length > 0) {
        CHAMELEON.CORE.handleTabChanges($tabsWrapper);
    }
});
